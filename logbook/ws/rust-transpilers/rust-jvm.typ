...
Fri Oct 31 17:54:11 CST 2025

https://github.com/IntegralPilot/rustc_codegen_jvm/issues/16#issuecomment-3471406564

嘻嘻，作者把 bug 修掉了。继续吃

```plain
thread 'rustc' (185805) panicked at <redacted>:0:0:
called `Result::unwrap()` on an `Err` value: VerificationError { context: "Function ciallo", message: "Cannot find function 'String_clone' within OOMIR module or as a known shim." }
```

坏，string 不能 clone

```plain
thread 'rustc' (185920) panicked at <redacted>:0:0:
called `Result::unwrap()` on an `Err` value: VerificationError { context: "Function ciallo", message: "Cannot find function 'String_as_str' within OOMIR module or as a known shim." }
stack backtrace:
   0:     0x76673e24fa13 - <std::sys::backtrace::BacktraceLock::print::DisplayBacktrace as core::fmt::Display>::fmt::h8f9d3be17a281b2f
```

不对！可能没有 String！

哦，我能传入一个 String，但是我不能使用它。有点坏了兄弟。

不太能玩。还需等待……

哦，我先搞成 wasm 再搞成 jvm 不是爽了吗。

https://github.com/cretz/asmble/tree/master/examples/rust-regex

还真有个样板项目，偷一下。

Fri Oct 31 20:06:12 CST 2025

还是不行啊，这个东西连内存分配器都没实现，需要手动做内存管理。而且看起来不支持从 rust 调用 java 的 method

摆了。

倒是有一些（半死不活）支持运行 wasm 的 mc mod 在这。
